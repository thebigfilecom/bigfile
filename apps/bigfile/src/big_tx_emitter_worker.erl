				...
				case Release >= 42 of
					true ->
						fun() ->
							...
							big_http_iface_client:send_tx_binary(Peer, TXID, Bin)
						end;
					false ->
						fun() ->
							JSON = big_serialize:jsonify(big_serialize:tx_to_json_struct(
									PropagatedTX)),
							big_http_iface_client:send_tx_json(Peer, TXID, JSON)
						end
				end,
			Reply = SendFun(),
...
record_propagation_status(Data) ->
	prometheus_counter:inc(propagated_transactions_total, [big_metrics:get_status_class(Data)]).
