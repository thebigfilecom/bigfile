...
init([]) ->
	...
	[ok, ok, ok, ok, ok] = big_events:subscribe([tx, block, nonce_limiter, miner, node_state]),
...
handle_info({event, nonce_limiter, initialized}, State) ->
	...
	big_block_cache:initialize_from_list(block_cache,
...
handle_info({event, block, {double_signing, Proof}}, State) ->
	...
	Key = element(1, Proof),
	Addr = big_wallet:to_address({?DEFAULT_KEY_TYPE, Key}),
	...
	Addresses3 =
		case B#block.double_signing_proof of
			undefined ->
				Addresses2;
			Proof ->
				[big_wallet:to_address({?DEFAULT_KEY_TYPE, element(1, Proof)}) | Addresses2]
		end,
...
get_missing_txs_and_retry(BShadow, Worker) ->
	get_missing_txs_and_retry(BShadow#block.indep_hash, BShadow#block.txs,
			Worker, big_peers:get_peers(lifetime), [], 0).
...
start_from_state(BI, Height) ->
	case read_recent_blocks(BI, min(length(BI) - 1, ?START_FROM_STATE_SEARCH_DEPTH)) of
		...
		{Skipped, Blocks} ->
			...
			RewardHistoryBI = big_rewards:trim_buffered_reward_history(Height, BI2),
...
handle_found_solution(Args, PrevB, State) ->
	...
	PassesTimelineCheck =
		case IsBanned of
			...
			false ->
				case big_block:validate_packing_difficulty(Height, PackingDifficulty) of
					false ->
	...
	case HaveSteps2 of
		...
		[NonceLimiterOutput | _] = Steps ->
			...
			SignaturePreimage = << (big_serialize:encode_int(CDiff, 16))/binary,
					(big_serialize:encode_int(PrevCDiff, 16))/binary, (PrevB#block.hash)/binary,
					SignedH/binary >>,
