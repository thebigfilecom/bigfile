...
verify_chunks({IntervalEnd, IntervalStart}, Intervals, State) ->
	...
	ChunkData = big_data_sync:get_chunk_by_byte({chunks_index, StoreID}, Cursor2+1),
...
verify_proof(MetaData, State) ->
	...
	case big_data_sync:read_data_path({chunk_data_db, StoreID}, ChunkDataKey) of
		{ok, DataPath} ->
...
verify_chunk_storage(PaddedOffset, ChunkSize, _Interval, State) ->
	...
	case big_data_sync:should_store_in_chunk_storage(PaddedOffset, ChunkSize, Packing) of
		true ->
...
invalidate_chunk(Type, Offset, ChunkSize, Logs, State) ->
	#state{ store_id = StoreID } = State,
	big_data_sync:invalidate_bad_data_record(Offset - ChunkSize, Offset, StoreID, 5),
