...
read_range2(MessagesRemaining, {Start, End, OriginStoreID, TargetStoreID, SkipSmall}) ->
	...
	case {big_sync_record:is_recorded(Start + 1, big_data_sync, OriginStoreID),
			big_data_sync:get_chunk_by_byte(ChunksIndex, Start + 1)} of
			...
			ReadChunk =
				case Skip of
					true ->
						skip;
					false ->
						big_data_sync:read_chunk(AbsoluteOffset, ChunkDataDB, ChunkDataKey,
								OriginStoreID)
				end,
			case ReadChunk of
				skip ->
		...
		true ->
			...
			case Start2 - 1 >= End of
				true ->
					ok;
				false ->
					case big_http_iface_client:get_chunk_binary(Peer, Start2, get_target_packing(TargetStoreID, State#state.request_packed_chunks)) of
						{ok, #{ chunk := Chunk } = Proof, _Time, _TransferSize} ->
							...
							Start3 = big_data_sync:get_chunk_padded_offset(
									Start2 + byte_size(Chunk)) + 1,
							Label = big_storage_module:label_by_id(TargetStoreID),
							gen_server:cast(list_to_atom("big_data_sync_" ++ Label),
...
get_target_packing(StoreID, true) -> big_storage_module:get_packing(StoreID);
