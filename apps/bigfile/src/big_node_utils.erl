...
%% are made in big_block_pre_validator.erl, big_nonce_limiter.erl, and
...
may_be_apply_double_signing_proof3(B, PrevB, Accounts) ->
	...
	EncodedCDiff1 = big_serialize:encode_int(CDiff1, 16),
	EncodedPrevCDiff1 = big_serialize:encode_int(PrevCDiff1, 16),
	...
	case big_wallet:verify(Key, SignaturePreimage1, Signature1) of
		...
		true ->
			EncodedCDiff2 = big_serialize:encode_int(CDiff2, 16),
			EncodedPrevCDiff2 = big_serialize:encode_int(PrevCDiff2, 16),
...
update_accounts4(B, PrevB, Accounts, Args) ->
	...
	case B#block.double_signing_proof of
		...
		Proof ->
			Denomination = PrevB#block.denomination,
			BannedAddr = big_wallet:to_address({?DEFAULT_KEY_TYPE, element(1, Proof)}),
...
test_block_validation() ->
	...
	[B0] = big_weave:init([{big_wallet:to_address(Pub), ?AR(200), <<>>}]),
	...
	PrevTX = big_test_node:sign_tx(main, Wallet, #{ reward => ?AR(10),
			data => crypto:strong_rand_bytes(10 * 1024 * 1024) }),
	big_test_node:assert_post_tx_to_peer(main, PrevTX),
	big_test_node:mine(),
	[_ | _] = big_test_node:wait_until_height(1),
	big_test_node:mine(),
	[{PrevH, _, _} | _ ] = big_test_node:wait_until_height(2),
	...
	big_test_node:mine(),
	[{H2, _, _} | _ ] = big_test_node:wait_until_height(4),
