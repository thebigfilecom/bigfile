...
setup_nodes2(#{ addr := MainAddr, peer_addr := PeerAddr } = Options) ->
	...
	[B0] = big_weave:init([{big_wallet:to_address(Pub), ?BIG(200000), <<>>}]),
	{ok, Config} = application:get_env(bigfile, config),
	case maps:get(storage_modules, Options, not_found) of
		not_found ->
			big_test_node:start(B0, MainAddr, Config);
		StorageModules ->
			big_test_node:start(B0, MainAddr, Config, StorageModules)
	end,
	{ok, PeerConfig} = big_test_node:remote_call(peer1, application, get_env, [bigfile, config]),
	big_test_node:start_peer(peer1, B0, PeerAddr, PeerConfig),
