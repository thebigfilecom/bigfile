...
test_single_node_one_chunk() ->
	...
	BI = big_test_node:wait_until_height(ValidatorNode, 1),
...
mine_in_parallel(Miners, ValidatorNode, CurrentHeight) ->
	...
	[{Hash, _, _} | _] = big_test_node:wait_until_height(ValidatorNode, CurrentHeight + 1),
	lists:foreach(
		fun(Node) ->
			[{MinerHash, _, _} | _] = big_test_node:wait_until_height(Node, CurrentHeight + 1),
			Message = lists:flatten(
				io_lib:format("Node ~p did not mine the same block as the validator node", [Node])),
			?assertEqual(big_util:encode(Hash), big_util:encode(MinerHash), Message)
		end,
		Miners
	),
	{ok, Block} = big_test_node:http_get_block(Hash, ValidatorNode),
