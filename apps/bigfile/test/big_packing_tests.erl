...
test_full_chunk() ->
	...
	ChunkSize = 256*1024,
	TXRoot = big_util:decode(?ENCODED_TX_ROOT),
	RewardAddress = big_test_node:load_fixture("big_packing_tests/address.bin"),
	...
	test_packs_chunks_depending_on_packing_threshold() ->
		...
		{LegacyProofs, StrictProofs, V1Proofs} = lists:foldl(
			fun(Height, {Acc1, Acc2, Acc3}) ->
				...
				{#tx{ id = TXID1 } = TX1, Chunks1} = big_test_data_sync:tx(Wallet, {fixed_data, DR1, Chunks1}),
				{#tx{ id = TXID2 } = TX2, Chunks2} = big_test_data_sync:tx(Wallet, {fixed_data, DR2, Chunks2}),
				{#tx{ id = TXID3 } = TX3, Chunks3} = big_test_data_sync:tx(Wallet, {fixed_data, DR3, Chunks3}, v1),
				{Miner, Receiver} =
				...
				?debugFmt("Mining block ~B, txs: ~s, ~s, ~s; data roots: ~s, ~s, ~s.~n", [Height,
						big_util:encode(TX1#tx.id), big_util:encode(TX2#tx.id),
						big_util:encode(TX3#tx.id), big_util:encode(TX1#tx.data_root),
						big_util:encode(TX2#tx.data_root), big_util:encode(TX3#tx.data_root)]),
				B = big_test_node:post_and_mine(#{ miner => Miner, await_on => Receiver }, [TX1, TX2, TX3]),
				big_test_data_sync:post_proofs(main, B, TX1, Chunks1),
				big_test_data_sync:post_proofs(peer1, B, TX2, Chunks2),
				{maps:put(TXID1, big_test_data_sync:get_records_with_proofs(B, TX1, Chunks1), Acc1),
						maps:put(TXID2, big_test_data_sync:get_records_with_proofs(B, TX2, Chunks2), Acc2),
						maps:put(TXID3, big_test_data_sync:get_records_with_proofs(B, TX3, Chunks3), Acc3)}
			end,
			{#{}, #{}, #{}},
			lists:seq(1, 20)
		),
		...
		lists:foldl(
			fun(Height, PrevB) ->
				...
				?debugFmt("Mined a block. "
						...
						big_util:encode(PrevB#block.indep_hash), PartitionUpperBound,
						BlockStart, BlockEnd, big_util:encode(TXRoot)]),
			...
			fun(TXID, [{_B, _TX, Chunks, _} | _] = Proofs) ->
				...
				case length(BigChunks) == Length
					...
					true ->
						...
						big_test_node:assert_get_tx_data(main, TXID, ExpectedData),
						big_test_node:assert_get_tx_data(peer1, TXID, ExpectedData),
						big_test_data_sync:wait_until_syncs_chunks([P || {_, _, _, P} <- Proofs]),
						big_test_data_sync:wait_until_syncs_chunks(peer1, [P || {_, _, _, P} <- Proofs], infinity);
					false ->
						?debugFmt("Asserting random split which turned out NOT strict"
								" and was placed above the strict data split threshold, "
								"TXID: ~s.", [big_util:encode(TXID)]),
						?debugFmt("Chunk sizes: ~p.", [[byte_size(Chunk) || Chunk <- Chunks]]),
						big_test_node:assert_data_not_found(main, TXID),
						big_test_node:assert_data_not_found(peer1, TXID)
				end
			end,
			LegacyProofs
		).
