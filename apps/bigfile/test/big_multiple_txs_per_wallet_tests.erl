-module(big_multiple_txs_per_wallet_tests).
...
-import(big_test_node, [wait_until_height/1, assert_wait_until_height/2,
...
joins_network_successfully() ->
	...
	big_test_node:disconnect_from(peer1),
	TX2 = big_test_node:sign_tx(Key, #{ last_tx => element(1, lists:nth(?MAX_TX_ANCHOR_DEPTH, BI)) }),
	big_test_node:assert_post_tx_to_peer(main, TX2),
	big_test_node:mine(),
	wait_until_height(?MAX_TX_ANCHOR_DEPTH + 1),
	TX3 = big_test_node:sign_tx(Key, #{ last_tx => element(1, lists:nth(?MAX_TX_ANCHOR_DEPTH, BI)) }),
	big_test_node:assert_post_tx_to_peer(peer1, TX3),
	big_test_node:mine(peer1),
	BI2 = assert_wait_until_height(peer1, ?MAX_TX_ANCHOR_DEPTH + 1),
	big_test_node:connect_to_peer(peer1),
	TX4 = big_test_node:sign_tx(Key, #{ last_tx => element(1, lists:nth(?MAX_TX_ANCHOR_DEPTH, BI2)) }),
	big_test_node:assert_post_tx_to_peer(peer1, TX4),
...
one_wallet_list_one_block_anchored_txs(Key, B0) ->
	...
	%% estimation from it.
	TX1Fun = fun() -> big_test_node:sign_v1_tx(Key, #{ reward => ?BIG(1) }) end,
	TX2Fun = fun() -> big_test_node:sign_v1_tx(Key, #{ reward => ?BIG(1),
...
two_block_anchored_txs(Key, B0) ->
	%% Sign only after the node has started to get the correct price
	%% estimation from it.
	TX1Fun = fun() -> big_test_node:sign_v1_tx(Key, #{ reward => ?BIG(1),
			last_tx => B0#block.indep_hash }) end,
	TX2Fun = fun() -> big_test_node:sign_v1_tx(Key, #{ reward => ?BIG(1),
			last_tx => B0#block.indep_hash }) end,
	[TX1Fun, TX2Fun].
