...
reset_node() ->
	...
	Key = element(1,
		big_test_node:remote_call(peer1, big_wallet, load_key, [Config#config.mining_addr])),
	{Key, B, PrevB}.
...
test_rejects_invalid_blocks() ->
	...
	{ok, Config} = big_test_node:remote_call(peer1, application, get_env, [bigfile, config]),
	Key = element(1, big_test_node:remote_call(peer1, big_wallet, load_key, [Config#config.mining_addr])),
...
test_reject_block_invalid_double_signing_proof() ->
	...
	{ok, Config} = big_test_node:remote_call(peer1, application, get_env, [bigfile, config]),
	ok = big_events:subscribe(block),
	{Key, _} = FullKey = big_test_node:remote_call(peer1, big_wallet, load_key, [Config#config.mining_addr]),
	...
	SignaturePreimage = << (big_serialize:encode_int(CDiff, 16))/binary,
					(big_serialize:encode_int(PrevCDiff, 16))/binary, Preimage2/binary >>,
	Signature2 = big_wallet:sign(Key, SignaturePreimage),
	...
	SignaturePreimage3 = << (big_serialize:encode_int(CDiff, 16))/binary,
					(big_serialize:encode_int(PrevCDiff, 16))/binary, Preimage3/binary >>,
	SignaturePreimage4 = << (big_serialize:encode_int(CDiff, 16))/binary,
					(big_serialize:encode_int(PrevCDiff, 16))/binary, Preimage4/binary >>,
	Signature3 = big_wallet:sign(Key, SignaturePreimage3),
	Signature4 = big_wallet:sign(Key, SignaturePreimage4),
	...
	big_test_node:connect_to_peer(peer1),
	wait_until_height(2),
	B6 = big_test_node:remote_call(peer1, big_storage, read_block, [hd(BI2)]),
