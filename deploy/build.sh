...
BASE_IMAGES=("bigfile-base:18.04" "bigfile-base:20.04" "bigfile-base:22.04" "")
...
for i in "${!LINUX_VERSIONS[@]}"; do
    ...
    IMAGE_NAME="bigfile:$VERSION-$LINUX_VERSION"
    OUTPUT_FILE="./output/bigfile-$VERSION.$LINUX_VERSION-x86_64.tar.gz"
    ...
    run_cmd "mv './output/bigfile.tar.gz' '$OUTPUT_FILE'"
done

if [ $PRE_RELEASE -eq 0 ]; then
  run_cmd "cp './output/bigfile-$VERSION.ubuntu22-x86_64.tar.gz' './output/bigfile-$VERSION.linux-x86_64.tar.gz'"
fi
